...
const List = ({ value, optionClick, noOptionsTitle, filterValue, filterValueText, ...rest }) => (
  <select
    className="pf-c-form-control pf-u-pr-sm ddorg__pf4-component-mapper__dual-list-select"
    multiple
    { ...rest }
  >
    { value.length < 1 && (<option className="ddorg__pf4-component-mapper__dual-list-select-option" disabled>
      { filterValue ? filterValueText : noOptionsTitle }
    </option>) }
    { value.length > 0 && value.map(({ value, label }) => (
...
const WrappedDualList = ({ FieldProvider, ...rest }) => (
  <FieldProvider
    { ...rest }
