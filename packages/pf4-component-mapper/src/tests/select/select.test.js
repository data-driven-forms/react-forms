...
describe('<Select />', () => {
  ...
  it('should render translated option in value container', async () => {
    ...
    expect(screen.getByText('Translated', { selector: 'h1' }).closest('.pf-v6-c-select__toggle-text')).toBeInTheDocument();
  });
  ...
  it('should render groups and dividers', async () => {
    ...
    expect(screen.getByText('Category 1')).toHaveClass('pf-v6-c-select__menu-group-title');
    expect(screen.getByText('Category 2')).toHaveClass('pf-v6-c-select__menu-group-title');
    expect(container.getElementsByClassName('pf-v6-c-divider')).toHaveLength(3);
    expect([...container.getElementsByClassName('pf-v6-c-select__menu-item')].map((opt) => opt.textContent)).toEqual([
      'value 1',
      'value 2',
      'independent 1',
      'value 3',
    ...
    expect(screen.getByText('value 1', { selector: 'button.pf-v6-c-select__menu-item' })).toHaveClass('pf-m-selected');
  ...
  it('filters with nested options', async () => {
    ...
    expect(container.getElementsByClassName('pf-v6-c-divider')).toHaveLength(0);
    expect(container.getElementsByClassName('pf-v6-c-select__menu-group-title')).toHaveLength(0);
  ...
  it('should return multiple simple values', async () => {
    ...
    await userEvent.click(screen.getByLabelText('open menu'));
    await userEvent.click(screen.getByText('First option', { selector: '.pf-v6-c-select__menu-item' }));
  ...
  it('should expand and close multi value chips', async () => {
    ...
    expect(container.querySelectorAll('.pf-v6-c-chip-group')).toHaveLength(1);
    expect(container.querySelectorAll('div.pf-v6-c-chip')).toHaveLength(3);

    await userEvent.click(screen.getByText('1 more'));

    expect(container.querySelectorAll('div.pf-v6-c-chip')).toHaveLength(4);
  });
  ...
  it('should load multi select Async options correctly and set initial value to ["123"]', async () => {
    ...
    await userEvent.click(screen.getByLabelText('open menu'));
    await waitFor(() => expect(screen.getByText('label', { selector: '.pf-v6-c-select__menu-item' })).toBeInTheDocument());
