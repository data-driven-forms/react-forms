...
describe('DualListSelect', () => {
  ...
  it('switch left option with holding ctrl', () => {
    ...
    wrapper.find('select').first().find('option').first().simulate('click');
    wrapper.update();
    wrapper.find('select').first().find('option').last().simulate('click', { ctrlKey: true });
    wrapper.update();
  ...
  it('switch left option with holding shift', () => {
    ...
    wrapper.find('select').first().find('option').first().simulate('click');
    wrapper.update();
    wrapper.find('select').first().find('option').last().simulate('click', { shiftKey: true });
    wrapper.update();
  ...
  it('switch left option with holding and removing by ctrl', () => {
    ...
    wrapper.find('select').first().find('option').last().simulate('click', { shiftKey: true });
    wrapper.update();
    wrapper.find('select').first().find('option').first().simulate('click', { ctrlKey: true });
    wrapper.update();
  ...
  it('switch all to right', () => {
    ...
    wrapper.find(AngleDoubleRightIcon).parent().props().onClick();
    wrapper.update();
  ...
  it('filters options', () => {
    ...
    expect(wrapper.find('select').first().find('option')).toHaveLength(schema.fields[0].options.length);

    wrapper.find('input').first().instance().value = 'cats';
    wrapper.find('input').first().simulate('change');
    wrapper.update();

    expect(wrapper.find('select').first().find('option')).toHaveLength(3);
    wrapper.find('select').first().find('option').forEach((option) => expect(option.text()).toEqual(expect.stringContaining('cats')));
  });

  it('filters value', () => {
    const wrapper = mount(<FormRenderer { ...initialProps } initialValues={{ 'dual-list': schema.fields[0].options.map(({ value }) => value) }}/>);

    expect(wrapper.find('select').last().find('option')).toHaveLength(schema.fields[0].options.length);

    wrapper.find('input').last().instance().value = 'cats';
    wrapper.find('input').last().simulate('change');
    wrapper.update();

    expect(wrapper.find('select').last().find('option')).toHaveLength(3);
    wrapper.find('select').last().find('option').forEach((option) => expect(option.text()).toEqual(expect.stringContaining('cats')));
  });

  it('sort options', () => {
    const wrapper = mount(<FormRenderer { ...initialProps }/>);

    expect(wrapper.find('select').first().find('option').first().text()).toEqual('cats');
    expect(wrapper.find('select').first().find('option').last().text()).toEqual('zebras');

    wrapper.find(SortAlphaUp).first().parent().props().onClick();
    wrapper.update();

    expect(wrapper.find('select').first().find('option').first().text()).toEqual('zebras');
    expect(wrapper.find('select').first().find('option').last().text()).toEqual('cats');

    wrapper.find(SortAlphaDown).first().parent().props().onClick();
    wrapper.update();
  ...
  it('sort value', () => {
    ...
    expect(wrapper.find('select').last().find('option').first().text()).toEqual('cats');
    expect(wrapper.find('select').last().find('option').last().text()).toEqual('zebras');

    wrapper.find(SortAlphaUp).last().parent().props().onClick();
    wrapper.update();

    expect(wrapper.find('select').last().find('option').first().text()).toEqual('zebras');
    expect(wrapper.find('select').last().find('option').last().text()).toEqual('cats');

    wrapper.find(SortAlphaDown).last().parent().props().onClick();
    wrapper.update();
