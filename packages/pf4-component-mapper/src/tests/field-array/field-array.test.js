...
describe('FieldArray', () => {
  ...
  it('should render array field correctly with props and initial values', () => {
    ...
    const wrapper = mount(
      <ContextWrapper initialValues={{ foo: [ 'bar', 'foo' ]}} schema={{ fields: formFields }}/>
    );
  ...
  it('should extract dynamic field name if it contains custom name', () => {
    ...
    const wrapper = mount(
      <ContextWrapper initialValues={{ foo: [ 'bar' ]}} schema={{ fields: formFields }}/>
    );
  ...
  it('should disable add buton when maxItem is reached', () => {
    ...
    expect(wrapper.find(AddCircleOIcon).props().className.includes('disabled')).toEqual(true);
  });
  ...
  it('should disable add buton when maxItem is reached', () => {
    ...
    expect(wrapper.find(CloseIcon)).toHaveLength(2);
    expect(wrapper.find(CloseIcon).first().props().className.includes('disabled')).toEqual(true);
    expect(wrapper.find(CloseIcon).last().props().className.includes('disabled')).toEqual(true);
  });
});
