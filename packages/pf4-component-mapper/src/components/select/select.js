...
export class Select extends React.Component {
  ...
  updateOptions = () => {
    ...
    return this.props.loadOptions().then((data) => {
      if (Array.isArray(this.props.value)) {
        const selectValue = this.props.value.filter((value) =>
        ...
        .then((options) => {
          if (this.isMounted) {
            this.setState((prevState) => ({
              ...
              allOptions: [...prevState.allOptions, ...options.filter(({ value }) => !prevState.allOptions.find((option) => option.value === value))]
            }));
          }
        })
        .catch((error) => {
  ...
  renderNoOptionsMessage = () =>
    Object.values(this.state.promises).some((value) => value) ? () => this.props.updatingMessage : () => this.props.noOptionsMessage;
    ...
    return (
      <Select
        ...
        onChange={(option) => {
          ...
          return simpleValue ? props.onChange(props.isMulti ? o.map((item) => item.value) : o ? o.value : undefined) : props.onChange(o);
        }}
